...
function App() {
  ...
  const getUsers = function (page) {
    var file = `https://reqres.in/api/users?page=1`
    return (
      fetch(file)
        .then(x => x.text())
        .then(
      ...
      <div className="container">
        <header>
          ...
          <div className='buttons'>
            <button type="button" className="btn" id="get-users" onClick={display}><i class="fa-solid fa-users"></i>{state ? 'Get Users' : 'Hide Users'}</button>
          </div>
        </header>
        <section>
          {
